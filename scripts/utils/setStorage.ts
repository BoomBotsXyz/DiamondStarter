...
const { hexlify, zeroPad, hexStripZeros, solidityKeccak256, getAddress } = ethers.utils;
...
export async function findERC20BalanceOfSlot(token: string, isVyper: boolean = false) {
  ...
  for(let slotNum = 0; slotNum < 100; ++slotNum) {
    await manipulateERC20BalanceOf(token, slotNum, dummyAddress, newBal, false, isVyper);
...
export function bytes32ToAddress(address:string) {
  return getAddress(`0x${address.substring(26,66)}`)
}
